<?php
    ...
    $db = Database::connect();
    $req = $db->query("SELECT * FROM utilisateur WHERE IdUti = ?");

    if(!empty($_POST)) {
        $errors = array();
        $id = checkInput($_POST['id']);
        $statement = $db->prepare("DELETE FROM utilisateur WHERE IdUti = ?");
         ...
         <div class="container admin">
            <div class="row">
                <?php 
                    ...
                    $reqU = $db->query("SELECT * FROM utilisateur where IdUti = $id");
                    while($donnees = $reqU->fetch()){
                        echo '<h1><strong>Supprimer l\'utilisateur '.$donnees['Prenom'].' ?</strong></h1>';
                    }
                ?>
                <br>
